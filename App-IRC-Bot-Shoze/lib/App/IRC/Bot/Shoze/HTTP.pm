...
use App::IRC::Bot::Shoze::HTTP::Network;
use App::IRC::Bot::Shoze::HTTP::User;
...
sub new {
    ...
    bless( $s, $class );
    $s->_parent($parent);
    $s->network( new App::IRC::Bot::Shoze::HTTP::Network($s) );
    $s->user( new App::IRC::Bot::Shoze::HTTP::User($s) );
    return $s;
}
...
#sub request_b {
#    my ( $s, $host, $port, $ressource, $action, $apikey, $apikey_private,
#         $format )
#      = @_;
#
