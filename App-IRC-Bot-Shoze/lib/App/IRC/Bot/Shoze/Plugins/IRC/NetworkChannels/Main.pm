    ...
    $s->cmd(
        {
           'channel_add' => {
                              ...
                              argument_filter =>
                                [$App::IRC::Bot::Shoze::String::regexp_channel],
           },
           'channel_del' => {
                              access           => 'msg',
                              lvl              => 800,
           ...
           'channel_set' => {
               access           => 'msg',
               lvl              => 500,
               help_cmd         => '!channel.set <channel> <key> <value>',
               help_description => 'Adding channel',
               argument_filter  => [
                   $App::IRC::Bot::Shoze::String::regexp_channel,
                   $App::IRC::Bot::Shoze::String::regexp_key,
                   # Dangerous filter, topic can be everything
                   $App::IRC::Bot::Shoze::String::regexp_everything
               ],
           },
           'channel_list' => {
                               access           => 'msg',
           ...
           'channel_set_owner' => {
               access           => 'msg',
               lvl              => 800,
               help_cmd         => '!channel.set.owner <channel_name> <owner>',
           ...
           'op' => {
                     ...
                     argument_filter =>  $App::IRC::Bot::Shoze::String::regexp_channel,
           ...
           'topic' => {
                        ...
                        argument_filter  => $App::IRC::Bot::Shoze::String::regexp_everything,
           }
        }
    );
    return $s;
...
sub channel_add {
    ...
    if ($Channel) {
        ...
        $Channel = $db->NetworkChannels->get_by( $irc->{Network},
                                           { type => $ctype, name => $cname } );
        $irc->{Out}
          ->join( $Session->get_hostmask, $Channel->_usable_name, $Channel );
    } else {
        return
...
sub channel_del {
    ...
    my ($channame) = ( split( /\s+/, $msg ) )[1];
    $channame =~ /^(#|&)([\w\d_]+)$/ or do {
        return $s->_n_error( $irc, $Session, "[$cmdname] Invalid syntax, " );
    };
    my $type;
    ( $type, $channame ) = ( $1, $2 );
    my $Channel = $db->NetworkChannels->get_by( $irc->{Network},
    ...
    $irc->{Out}
      ->part( $Session->get_hostmask, $Channel->_usable_name, $Channel );
...
sub channel_info {
    ...
    $out .= " - topic________: "
      . ( $Channel->wanted_topic ? $Channel->wanted_topic : "" ) . "\n";
...
sub channel_set {
    ...
    unless ( grep /^$key$/, @akeys ) {
        return $s->_n_error( $irc, $Session, "[$cmdname] Invalid key $key" );
    }
...
sub channel_set_owner {
    ...
    if ( $Channel->owner ) {
             ...
             and ( $CurrentOwner->lvl >= $Session->user_lvl ) )
        {
            ...
            $irc->{Out}
              ->notice( '#me#', $Session->nick => "[$cmdname] Same owner!" );
            return PCI_EAT_ALL;
        }
    }
    ...
    if ( $Channel->_update ) {
        ...
        $irc->{Out}->notice( '#me#',
                                 $Session->nick => "[$cmdname] Cannot set "
                               . $Channel->_usable_name
                               . " owner set to "
                               . $Owner->name );
        return PCI_EAT_ALL;
